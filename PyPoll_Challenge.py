...
with open(file_to_save, "w") as txt_file:
    ...
    for county_name in county_votes:
        ...
        # print(f"{county_name}: {cvotes_percentage:.1f}% ({cvotes:,})\n")
        county_results = (f"{county_name}: {cvotes_percentage:.1f}% ({cvotes:,})\n")
        print(county_results)
         # 6e: Save the county votes to a text file.
        txt_file.write(county_results)
         # 6f: Write a decision statement to determine the winning county and get its vote count.
        if (cvotes > most_votes) and (cvotes_percentage > high_percentage):
            most_votes = cvotes
            largest_turnout = county_name
            high_percentage = cvotes_percentage

    # 7: Print the county with the largest turnout to the terminal.
    largest_turnout_summary = (
        f"_________________________\n"
        f"Largest County Turnout: {largest_turnout}\n"
        f"Largest County Turnout County Vote Count: {most_votes:,}\n"
        f"Largest County Turnout County Vote Percentage of Total Votes: {high_percentage:.1f}%\n"
        f"-------------------------\n")
